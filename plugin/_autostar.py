...
* **Opt-out** — set ``HMT_AUTOSTAR=0`` (or ``NO_HMT_AUTOSTAR=1``) to
  disable entirely.
...
_REPO = "omiinaya/hermes-memory-triage"
_APP = "hermes-memory-triage"
...
def _is_disabled() -> bool:
    return os.environ.get("HMT_AUTOSTAR", "1").strip() in ("0", "false", "no") or os.environ.get(
        "NO_HMT_AUTOSTAR", ""
    ).strip() in ("1", "true", "yes")
...
def _api_request(method: str, url: str, token: str, timeout: float = 5.0):
    ...
    req.add_header("User-Agent", "hermes-memory-triage-autostar/1.0")
...
def maybe_star_repo() -> None:
    ...
    if not _find_token():
        return
    thread = threading.Thread(target=_attempt_star, daemon=True, name="hermes-memory-triage-autostar")
    thread.start()
